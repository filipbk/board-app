...
export class CommentController {
    ...
    @Get(':offerId')
    getComments(@Param('offerId') id: number, @CurrentUser() currentUser: TokenUserData) {
        return this.service.getCommentsByOfferId(id, currentUser);
    }

    @UseGuards(JwtAuthGuard)
    @Get('/:offerId/all')
    getAllComments(@Param('offerId') id: number, @CurrentUser() currentUser: TokenUserData) {
        return this.service.getAllCommentsByOfferId(id, currentUser)
    }

    @UseGuards(JwtAuthGuard)
    @Post()
    createComment(@Body() comment: CommentCreateDto, @CurrentUser() currentUser: TokenUserData,
    ) {
